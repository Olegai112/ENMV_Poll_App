...
class CalibView():
    ...
    def __init__(self, master):
        ...
        self.calib_pause_between_requests = None
    ...
    def connect_to_calib_thread(self, precision_research_flag = None, callback = None ,return_state = None, calib_disc_event=None):
        self.connect_to_calib_tread = threading.Thread(target=lambda: self.connect_to_calib(callback = callback, return_state = return_state, calib_disc_event =calib_disc_event), daemon=True)
        ...
        if precision_research_flag != None:
            if precision_research_flag:
                self.calib_connected_flag = True
            else:
                self.calib_connected_flag = False
        else:
            self.calib_connected_flag = self.calib_status_chbtn_var.get()

        self.connect_to_calib_tread.start()

    def connect_to_calib(self, callback = None, return_state = None, calib_disc_event = None):
        if self.calib_connected_flag:
            while self.calib_connected_flag:
                try:
            ...
            try:
                while self.calib_connected_flag:
                    response = self.calib.send_response([166] * 8)
                    if response == b'':
                        if callback == None:
                            sleep(1)
                        else:
            ...
            try:
                while True:
                    if self.calib.pause_between_requests or self.calib.pause_between_requests == None:
                        self.calib.disconnect()
                        self.calib.client = None
                        if self.calib.pause_between_requests != None:
                            self.calib.stop_calib_requests_event.wait()
                        break
                    else:
                        sleep(0.1)
            except Exception as e:
        ...
        if self.calib.client:
            ...
            if return_state != None:
                return_state(device_disconnect_flag = "device_disconnect_flag")
            self.calib_status_chbtn_var.set(value = False)

        if calib_disc_event != None:
            calib_disc_event.set()
    ...
    def add_set_vidgets(self):
        ...
        self.to_set_value_sb = ttk.Spinbox(self.calib_mode_frame, from_=-100.0, to=100.0, increment=1, justify="center", validate="key", validatecommand=(vcmd, '%S', '%d', '%s', '%P'))
        self.to_set_value_sb.set(value=0)
        ...
        try:
            ...
            while self.calib_connected_flag:
                if self.chosen_mode == "Измерение":
                    ...
                    if counter % 10 == 0:
                        to_calibrate_flag = True
                    if to_calibrate_flag:
                        command = self.calib.measure_value(1)
                        to_calibrate_flag = False
                    else:
                        command = self.calib.measure_value()
                    try:
                        self.master.after(0, lambda: self.measure_value_label.config(text = self.responce))
                    except:
                        pass
                    ...
                    if hasattr(self, "set_value_label"):
                        self.master.after(0, lambda: self.set_value_label.config(text = self.responce))

                self.responce = round(self.calib.send_response(command)[1], 6)
                sleep(0.5)
        except Exception as e:
            print(f"calibcalib{e}")
            self.calib.client = None
            return
